...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType = {
    val fields = new Array[StructField](columnNames.length)
    fields(0) = StructField(columnNames.head, StringType, false)

    for ((header, i) <- columnNames.tail.zipWithIndex) {
      fields(i + 1) = StructField(header, DoubleType, false)
    }
    StructType(fields)
  }
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    val (primary, nonPrimary) = columnNames.partition(("""^(t01|t03|t11|t1801|t1803)"""r).findFirstIn(_).isDefined)
    val (working, nonWorking) = nonPrimary.partition(("""^(t05|t1805)"""r).findFirstIn(_).isDefined)
    val other = nonWorking.filter(("""^(t02|t04|t06|t07|t08|t09|t10|t12|t13|t14|t15|t16|t18)"""r).findFirstIn(_).isDefined)
  ...
  def timeUsageSummary(
    ...
    val workingStatusProjection: Column =
      when($"telfs".between(1,2), "working").otherwise("not working").as("working")
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    val grouped = summed.groupByKey( row => (row.working, row.sex, row.age))
    val aggregated = grouped.agg(avg(_.primaryNeeds), avg(_.work), avg(_.other))

    def round(value: Double) = Math.round(value * 10) / 10d

    aggregated.map {
      case ((working, sex, age), primaryNeeds, work, other) =>
        TimeUsageRow(working, sex, age, round(primaryNeeds), round(work), round(other))
    }.orderBy($"working", $"sex", $"age")
