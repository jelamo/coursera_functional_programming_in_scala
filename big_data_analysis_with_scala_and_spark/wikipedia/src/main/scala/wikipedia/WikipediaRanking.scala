...
object WikipediaRanking {
  ...
  val conf: SparkConf = new SparkConf().setMaster("local").setAppName("WikipediaAssignment")
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] = {
    rdd.flatMap( article => {
      langs.flatMap( lang => {
        article.text.split(" ").contains(lang) match {
          case true => Option((lang, article))
          case false => None
        }
      })
  ...
  def rankLangsUsingIndex(index: RDD[(String, Iterable[WikipediaArticle])]): List[(String, Int)] = {
    index.map(indexEntry => (indexEntry._1, indexEntry._2.size )).collect().toList.sortBy(_._2).reverse
  }
