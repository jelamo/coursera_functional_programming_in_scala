 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("sum of one number equals that number") {
    assert(sum(List(3)) === 3)
  }

  test("sum of negative numbers is a negative number") {
    assert(sum(List(-3, -4, -7)) === -14)
  }

  test("sum of negative and positive numbers with the same value cancel each other") {
    assert(sum(List(-3, 3, 5, -5)) === 0)
  }
  ...
  test("max of negative numbers is the one with the value closest to zero") {
    assert(max(List(-3, -4, -1, -8)) === -1)
  }

  test("max of repeated elements is one of them") {
    assert(max(List(4, 4, 4, 4)) === 4)
  }

  test("max of a list that includes both negative and positive numbers is always the biggest positive") {
    assert(max(List(-43, 8, -13, 3)) === 8)
  }

  test("max of a list with a single element is that single element") {
    assert(max(List(5)) === 5)
  }

  test("max of empty list throws 'java.util.NoSuchElementException") {
